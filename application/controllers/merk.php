...
class Merk extends CI_Controller {
		...
		if($_POST){
			...
			if($status == "baru"){
				...
				if($result == 1){
					...
					$this->session->set_flashdata("alert", "<div class='alert alert-danger'><strong>Data Gagal Disimpan</strong></div>");
					header('location:'.base_url().'merk');
				}
			}else{
				$data = array(
				...
				if($result == 1){
					$this->session->set_flashdata("sukses", "<div class='alert alert-success'><strong>Update Data Berhasil</strong></div>");
					header('location:'.base_url().'merk');
				}else{
					$this->session->set_flashdata("alert", "<div class='alert alert-danger'><strong>Update Data Gagal</strong></div>");
					header('location:'.base_url().'merk');
				}
			}
		}else{
	...
	function hapusmerk($kode = 1){
		...
		if($result == 1){
			$this->session->set_flashdata("sukses", "<div class='alert alert-success'><strong>Hapus data Berhasil</strong></div>");
			header('location:'.base_url().'merk');
		}else{
			$this->session->set_flashdata("alert", "<div class='alert alert-danger'><strong>Hapus data Gagal</strong></div>");
			header('location:'.base_url().'merk');
		}
	}
}
